"""Agent architecture for simple environments (no toxin input, no attack output).

This is a simplified version for thermotaxis/pretrain experiments.
- INPUT_DIM = 6: food, temp, 4 contact sensors (no toxin)
- OUTPUT_DIM = 6: eat, forward, left, right, stay, reproduce (no attack)
"""
...
# Architecture constants - SIMPLIFIED (no toxin input, no attack output)
INPUT_DIM = 6  # food, temp, 4 contact sensors (no toxin)
OUTPUT_DIM = 6  # eat, forward, left, right, stay, reproduce (no attack)
...
def compute_param_dim(hidden_dim: int, internal_noise_dim: int) -> int:
    ...
    # LSTM1: 4 gates x (input_dim x hidden + hidden x hidden + hidden bias)
    lstm1_size = 4 * (INPUT_DIM * hidden_dim + hidden_dim * hidden_dim + hidden_dim)

    # LSTM2: 4 gates x (input_dim_2 x hidden + hidden x hidden + hidden bias)
    lstm2_size = 4 * (input_dim_2 * hidden_dim + hidden_dim * hidden_dim + hidden_dim)

    # Output: hidden x output + output bias
...
def agent_forward_fully_flat(flat_params: jax.Array, flat_state: jax.Array, obs: jax.Array,
    ...
    Args:
        ...
        obs: Observation (6,) - food, temp, 4 contact sensors
    ...
    Returns:
        new_flat_state (4 * hidden_dim,), logits (6,)
    """
    h = hidden_dim
